...
{
    // Execute a simple SQL query
    const char *sql = "SELECT isAdmin FROM Users WHERE RFID = ?;";
    sqlite3_stmt *stmt;
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
        DB_info->isAdmin = sqlite3_column_int(stmt, 0);
        
        if (DB_info->isAdmin != 0)
        { 
            sqlite3_finalize(stmt);
            return 1;
        }
...
int DB_add_user(int32_t nuid)
{
    int rc;
    // Execute a simple SQL query
    const char *sql = "INSERT INTO users (rfid,name,sold,isAdmin) VALUES (?,?,?,?);";
    sqlite3_stmt *stmt;
...
int DB_add_sold(int32_t nuid, double amount)
{
    ...
    // Update the amount with the correct sum
    const char *sql = "UPDATE users SET sold = ? WHERE RFID = ?;";
    sqlite3_stmt *stmt;
